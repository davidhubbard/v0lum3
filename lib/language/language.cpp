...
// static const char VK_LAYER_LUNARG_core_validation[] =
// "VK_LAYER_LUNARG_core_validation";  static const char
// VK_LAYER_LUNARG_object_tracker[] = "VK_LAYER_LUNARG_object_tracker";  static
// const char VK_LAYER_LUNARG_parameter_validation[] =
// "VK_LAYER_LUNARG_parameter_validation";  static const char
// VK_LAYER_LUNARG_image[] = "VK_LAYER_LUNARG_image";  static const char
...
struct InstanceInternal : public Instance {
  ...
  int initSupportedQueues(std::vector<VkQueueFamilyProperties>& vkQFams,
                          Device& dev) {
    ...
    for (size_t q_i = 0; q_i < vkQFams.size(); q_i++) {
      ...
      VkResult v = vkGetPhysicalDeviceSurfaceSupportKHR(
          dev.phys, q_i, this->surface, &isPresentSupported);
      if (v != VK_SUCCESS) {
        fprintf(stderr,
                "dev %zu qfam %zu: vkGetPhysicalDeviceSurfaceSupportKHR "
                "returned %d (%s)\n",
...
int Instance::ctorError(const char** requiredExtensions,
  ...
  if (v != VK_SUCCESS) {
    fprintf(stderr,
            "createWindowSurface (the user-provided fn) failed: %d (%s)", v,
            string_VkResult(v));
    return 1;
  }
