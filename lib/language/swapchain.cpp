...
VkExtent2D calculateSurfaceExtend2D(const VkSurfaceCapabilitiesKHR& scap,
  ...
  return {
      /*width:*/ std::max(lo.width,
                          std::min(hi.width, surfaceSizeRequest.width)),
      /*height:*/
      std::max(lo.height, std::min(hi.height, surfaceSizeRequest.height)),
  };
}
...
int Device::resetSwapChain(VkSurfaceKHR surface, VkExtent2D sizeRequest) {
  ...
  if (v != VK_SUCCESS) {
    fprintf(stderr,
            "vkGetPhysicalDeviceSurfaceCapabilitiesKHR() returned %d (%s)\n", v,
            string_VkResult(v));
    return 1;
  }
