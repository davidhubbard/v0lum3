...
std::vector<VkExtensionProperties>* getExtensions() {
  ...
  if (r != VK_SUCCESS) {
    fprintf(stderr,
            "vkEnumerateInstanceExtensionProperties(count) returned %d (%s)\n",
            r, string_VkResult(r));
    return nullptr;
  }
  ...
  if (r != VK_SUCCESS && r != VK_INCOMPLETE) {
    fprintf(stderr,
            "vkEnumerateInstanceExtensionProperties(all) returned %d (%s)\n", r,
            string_VkResult(r));
    delete extensions;
    return nullptr;
  }
  if (extensionCount > extensions->size()) {
    ...
    // calls.
    fprintf(stderr,
            "vkEnumerateInstanceExtensionProperties(all) returned count=%u, "
            "larger than previously (%zu)\n",
...
std::vector<VkLayerProperties>* getLayers() {
  ...
  if (r != VK_SUCCESS) {
    fprintf(stderr,
            "vkEnumerateInstanceLayerProperties(count) returned %d (%s)\n", r,
            string_VkResult(r));
    return nullptr;
  }
  ...
  if (r != VK_SUCCESS && r != VK_INCOMPLETE) {
    fprintf(stderr,
            "vkEnumerateInstanceLayerProperties(all) returned %d (%s)\n", r,
            string_VkResult(r));
    delete layers;
    return nullptr;
  }
  if (layerCount > layers->size()) {
    ...
    fprintf(stderr,
            "vkEnumerateInstanceLayerProperties(all) returned count=%u, larger "
            "than previously (%zu)\n",
...
std::vector<VkPhysicalDevice>* getDevices(VkInstance instance) {
  ...
  if (devCount > devs->size()) {
    ...
    fprintf(stderr,
            "vkEnumeratePhysicalDevices(all) returned count=%u, larger than "
            "previously (%zu)\n",
            devCount, devs->size());
    delete devs;
    return nullptr;
...
std::vector<VkQueueFamilyProperties>* getQueueFamilies(VkPhysicalDevice dev) {
  ...
  if (qCount > qs->size()) {
    ...
    // calls.
    fprintf(stderr,
            "vkGetPhysicalDeviceQueueFamilyProperties(all) returned count=%u, "
            "larger than previously (%zu)\n",
            qCount, qs->size());
    delete qs;
    return nullptr;
...
std::vector<VkExtensionProperties>* getDeviceExtensions(VkPhysicalDevice dev) {
  ...
  if (r != VK_SUCCESS) {
    fprintf(stderr,
            "vkEnumerateDeviceExtensionProperties(count) returned %d (%s)\n", r,
            string_VkResult(r));
    return nullptr;
  }
  auto* extensions = new std::vector<VkExtensionProperties>(extensionCount);
  ...
  if (r != VK_SUCCESS && r != VK_INCOMPLETE) {
    fprintf(stderr,
            "vkEnumerateDeviceExtensionProperties(all) returned %d (%s)\n", r,
            string_VkResult(r));
    delete extensions;
    return nullptr;
  }
  if (extensionCount > extensions->size()) {
    ...
    // calls.
    fprintf(stderr,
            "vkEnumerateDeviceExtensionProperties(all) returned count=%u, "
            "larger than previously (%zu)\n",
            extensionCount, extensions->size());
    delete extensions;
    return nullptr;
...
std::vector<VkSurfaceFormatKHR>* getSurfaceFormats(VkPhysicalDevice dev,
  ...
  if (formatCount > formats->size()) {
    ...
    fprintf(stderr,
            "vkGetPhysicalDeviceSurfaceFormatsKHR(all) returned count=%u, "
            "larger than previously (%zu)\n",
...
std::vector<VkPresentModeKHR>* getPresentModes(VkPhysicalDevice dev,
  ...
  if (r != VK_SUCCESS) {
    fprintf(
        stderr,
        "vkGetPhysicalDeviceSurfacePresentModesKHR(count) returned %d (%s)\n",
        r, string_VkResult(r));
    return nullptr;
  }
  ...
  if (r != VK_SUCCESS && r != VK_INCOMPLETE) {
    fprintf(stderr,
            "vkGetPhysicalDeviceSurfacePresentModesKHR(all) returned %d (%s)\n",
            r, string_VkResult(r));
    delete modes;
    return nullptr;
  }
  if (modeCount > modes->size()) {
    ...
    fprintf(stderr,
            "vkGetPhysicalDeviceSurfacePresentModesKHR(all) returned count=%u, "
            "larger than previously (%zu)\n",
...
std::vector<VkImage>* getSwapchainImages(VkDevice dev,
  ...
  if (imageCount > images->size()) {
    ...
    fprintf(stderr,
            "vkGetSwapchainImagesKHR(all) returned count=%u, larger than "
            "previously (%zu)\n",
