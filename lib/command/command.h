...
typedef struct Shader {
	Shader(language::Device& dev)
		: vkdev{dev.dev}
		, vk{dev.dev, vkDestroyShaderModule}
	{
...
// written to VkAttachmentReference refvk here. The refvk is then added to the
...
protected:
	...
	WARN_UNUSED_RESULT virtual int init(
		language::Device& dev,
		RenderPass& renderPass,
		size_t subpass_i);
} Pipeline;
...
public:
	...
	WARN_UNUSED_RESULT int beginRenderPass(RenderPass& pass,
		...
		if (!pass.passBeginInfo.framebuffer) {
			fprintf(stderr, "beginRenderPass: framebuffer was not set to one of "
				"Device::framebufs.\n");
			return 1;
		}
	...
	WARN_UNUSED_RESULT int setDepthBias(
	...
	WARN_UNUSED_RESULT int setScissor(
	...
	WARN_UNUSED_RESULT int setViewport(RenderPass& pass) {
		...
		for (auto& pipe : pass.pipelines) {
			viewports.insert(viewports.end(),
				pipe.info.viewports.begin(), pipe.info.viewports.end());
		}
	...
	WARN_UNUSED_RESULT int setScissor(RenderPass& pass) {
		std::vector<VkRect2D> scissors;
		for (auto& pipe : pass.pipelines) {
			scissors.insert(scissors.end(),
				pipe.info.scissors.begin(), pipe.info.scissors.end());
		}
