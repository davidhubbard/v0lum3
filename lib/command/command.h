...
// the VkRenderPassCreateInfo, it is given an index so that an associated
// VkAttachmentReference can be added to the PipelineStage.
//
// TODO: dedup the VkAttachmentDescription values in RenderPass so that the
// caller can just add PipelineAttachments everywhere and the optimal layout
...
typedef struct PipelineCreateInfo {
	...
	// The VkAttachmentReference will be written to subpassDesc by init() with its
	// "attachment" index populated with the index into RenderPass:rpci where
	// the VkAttachmentDescription is located.
...
typedef struct RenderPass {
	...
	// something init() can set for you. And YOU MUST set
	// passBeginInfo.renderArea.extent any time the window is resized or anything
	// similar.
	...
	// passBeginClearColors is used by init() to populate passBeginInfo.
	// Calling setClearColor() after init() will have no effect.
